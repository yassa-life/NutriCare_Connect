# Module 05: Setup, Download & Testing Guide
...
1. **Phase 1: Full Project Setup & Test Run**: How to download the complete working repository from GitHub, set up dependencies (Java 21, Node.js, MySQL/Docker), run the system, and verify Module 05 (Messaging & Reminders).
...
## 🧪 Testing Module 05 (Messaging & Reminders Features)

As owner of **Module 05**, test the following key workflows on `http://localhost:5173`:

1. **Patient to Healthcare Staff Direct Messaging**:
   - Sign in as a patient (`patient@nutricare.demo` / `password`).
   - Open **Messages**, select a doctor or dietitian, and send a message.
2. **Doctor/Staff Reply**:
   - Sign in as doctor (`doctor@nutricare.demo` / `password`).
   - Open conversation inbox and reply to the patient's inquiry.
3. **Simulated Notification Delivery**:
   - Trigger an appointment or check-up reminder.
   - Verify simulated delivery logs displayed in the notification center widget.
4. **Reminder Management**:
   - Schedule a recurring daily diet/medication reminder and verify active reminder list.
...
2. Copy your feature files from `05-messaging-reminders-IT25103601` into your workspace.
...
```bash
# Test Module 05 backend code:
mvn -pl 05-messaging-reminders-IT25103601/backend -am test
