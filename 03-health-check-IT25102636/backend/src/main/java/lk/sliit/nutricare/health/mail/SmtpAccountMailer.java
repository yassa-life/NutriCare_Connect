...
class SmtpAccountMailer implements AccountMailer {
    ...
    SmtpAccountMailer(EmailDeliveryAttemptRepository deliveries, JavaMailSender mailSender,
        ...
        if (liveEmail && (this.from.isBlank() || username.isBlank() || password.isBlank()))
            throw new IllegalStateException("Live email requires MAIL_FROM, MAIL_USERNAME and MAIL_PASSWORD.");
    }

    @Override
    @Transactional(propagation = Propagation.REQUIRES_NEW, noRollbackFor = IllegalStateException.class)
    public void sendPasswordResetOtp(String email, String otp) {
        send(email, "PASSWORD_RESET_OTP", "NutriCare password reset code",
    ...
    public void sendStaffWelcome(String email) {
        send(email, "STAFF_ACCOUNT_CREATED", "Your NutriCare staff account",
                "Your NutriCare staff account is ready. Obtain the one-time password from your administrator, then change it immediately after signing in.",
    ...
    private void send(String email, String template, String subject, String body, String preview) {
        ...
        try {
            ...
            throw new IllegalStateException("Email could not be delivered. Check the SMTP configuration.", exception);
        }
    }
}
