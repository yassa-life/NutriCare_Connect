...
public class SecurityConfig {
    ...
    @Bean
    CorsConfigurationSource cors(@Value("${nutricare.web-origin:http://localhost:5173}") String origin) {
        CorsConfiguration configuration = new CorsConfiguration();
        List<String> configuredOrigins = Stream.of(origin.split(","))
                .map(String::trim)
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain)
        ...
        if (header != null && header.startsWith("Bearer ")) {
            try {
                ...
                Optional<UserAccount> authenticated = users.findById(claims.userId())
