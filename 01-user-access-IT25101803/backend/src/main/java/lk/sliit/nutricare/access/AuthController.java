...
public class AuthController {
    ...
    public Map<String, Object> register(@Valid @RequestBody RegisterRequest request) {
        UserAccount user = auth.register(request.fullName(), request.email(), request.password(),
                request.phoneNumber(), request.dateOfBirth());
        return Map.of("token", auth.login(user.getEmail(), request.password()), "user", UserView.of(user));
    }
    ...
    public Map<String, Object> forgotPassword(@Valid @RequestBody ForgotPasswordRequest request) {
        ...
        response.put("message", "If the account exists, a six-digit reset code has been sent by email.");
    ...
    public UserView changePassword(Principal principal, @Valid @RequestBody ChangePasswordRequest request) {
        return UserView.of(auth.changePassword(currentUserId(principal), request.currentPassword(), request.newPassword()));
    }
    ...
    @PreAuthorize("hasRole('SYSTEM_ADMIN')")
    @ResponseStatus(HttpStatus.CREATED)
    public ProvisionedStaffView provisionStaff(Principal principal, @Valid @RequestBody StaffRequest request) {
        AuthService.ProvisionedAccount account = auth.provisionStaff(currentUserId(principal), request.fullName(),
    ...
    @PreAuthorize("hasRole('SYSTEM_ADMIN')")
    public UserView status(Principal principal, @PathVariable String id, @RequestBody StatusRequest request) {
        return UserView.of(auth.setAccountStatus(currentUserId(principal), id, request.enabled()));
    }

    @PatchMapping("/users/{id}/role")
    @PreAuthorize("hasRole('SYSTEM_ADMIN')")
    public UserView role(Principal principal, @PathVariable String id, @RequestBody RoleRequest request) {
    ...
    public record ChangePasswordRequest(@NotBlank String currentPassword,
                                        @Size(min = 8, max = 72) String newPassword) {}
    public record StaffRequest(@NotBlank String fullName, @Email @NotBlank String email, @NotBlank String role) {}
    ...
    public record UserView(String id, String fullName, String email, String phoneNumber, LocalDate dateOfBirth,
        ...
        static UserView of(UserAccount user) {
            return new UserView(user.getId(), user.getFullName(), user.getEmail(), user.getPhoneNumber(),
                    ...
                    user.getFailedAttempts(), user.isMustChangePassword());
        }
    }
